...
- the OpenTUI run-log pane loads the active run log once per run-log path and then stays current from bridge-delivered incremental log events instead of polling and rereading the full log file
...
- guarded workspace tools resolve target paths against the workspace root and reject symlink escapes before execution
- blocked command patterns fail immediately after shell-segment and leading-command matching rather than raw substring checks
...
| OpenTUI incremental run logs | OpenTUI loads each active run log once per log path, merges same-path history with bridge-delivered incremental run-log events, and avoids duplicate log cards when file-backed history overlaps live bridge updates. | `src/runtime_app.zig` test: `logRuntimeEventWithUi emits incremental run log bridge events`; `opentui/runtime-log.test.ts` tests: `runtime log file entries render with the same identity as bridge updates`; `merging loaded and incremental runtime log entries avoids duplicates` |
| Runtime tool boundary hardening | Guarded workspace tools reject symlink escapes after resolved-root validation, `search_text` treats leading-dash patterns as literal data, and blocked shell patterns match actual shell command segments instead of raw substrings. | `src/runtime_app.zig` tests: `commandIsBlocked matches blocked commands at shell segment boundaries`; `searchText treats leading dash patterns as literal data`; `executeToolRequests blocks symlink escapes for guarded workspace tools` |
